...
const COLOR_LABELS = ['red', 'orange', 'yellow', 'blue', 'green'];
...
export function createQuestionCard(question, scoreManager) {
  ...
  function renderButtons() {
    ...
    STEPS.forEach(({ value, label, key }, idx) => {
      const color = COLOR_LABELS[idx];
  ...
  function renderSubScore() {
    ...
    if (selectedPct === null) {
      ...
      const formatted = Number.isInteger(earned) ? earned : earned.toFixed(2);
      el.textContent = `→ Score estimé : ${formatted} / ${question.bareme} pt${question.bareme > 1 ? 's' : ''}`;
    }
  }
  ...
  function toggleCorrige() {
    ...
    const toggleBtn = card.querySelector('.toggle-corrige');
    ...
    if (toggleBtn) {
      toggleBtn.textContent = corrigeVisible ? 'Masquer le corrigé' : 'Afficher le corrigé';
      toggleBtn.prepend(arrow);
    }
    if (arrow) {
      arrow.style.transform = corrigeVisible ? 'rotate(90deg)' : 'rotate(0deg)';
    }
  }
  ...
  card.innerHTML = `
    <div class="p-5">
      <div class="flex items-start justify-between gap-3 mb-3">
        <div class="flex items-center gap-2 flex-wrap min-w-0">
          ${question.partie
            ...
            : ''}
          <span class="text-sm font-bold text-gray-800 truncate">${question.numero}</span>
        </div>
      ...
      <div class="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap mb-4">${question.enonce}</div>
      ...
      <div class="corrige-block hidden mb-4">
        <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
          <p class="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-2">Corrigé</p>
          <div class="corrige-content">${question.corrige}</div>
        </div>
      </div>
